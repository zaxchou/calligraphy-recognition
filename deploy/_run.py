...
PEM = r"z:\BaiduSync\BaiduSyncdisk\molin-wiki\cali_cloud_20260503.pem"
HOST = "ubuntu@124.223.17.29"
DATA_DIR = r"z:\BaiduSync\BaiduSyncdisk\molin-wiki\backend\data"
...
with zipfile.ZipFile(zpath, 'w', zipfile.ZIP_DEFLATED) as zf:
    for root, dirs, files in os.walk(DATA_DIR):
        ...
        for f in files:
            ...
            arc = os.path.relpath(fp, r"z:\BaiduSync\BaiduSyncdisk\molin-wiki\backend")
            try:
                zf.write(fp, arc)
            except:
                pass
...
# Step 3: Stop backend, unzip into /opt/molin-wiki/backend, restart
cmds = [
    ...
    "echo '=== Unzipping data ==='",
    "sudo rm -rf /opt/molin-wiki/backend/data_old 2>/dev/null; sudo mv /opt/molin-wiki/backend/data /opt/molin-wiki/backend/data_old 2>/dev/null; true",
    "sudo mkdir -p /opt/molin-wiki/backend/data",
    "sudo unzip -o /tmp/data_upload.zip -d /opt/molin-wiki/backend/ 2>&1 | tail -5",
    "sudo chown -R ubuntu:ubuntu /opt/molin-wiki/backend/data/",
    "echo '=== Restarting backend ==='",
    "cd /opt/molin-wiki/deploy && sudo docker compose up -d --build backend 2>&1 | tail -5",
    "sleep 5",
