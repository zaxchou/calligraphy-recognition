...
### 3.8 i18n 决策 P2
vue-i18n 只有 3 个文件用了 $t，语言切换按钮形同虚设——比不做更糟。**建议做减法**：移除 vue-i18n 与切换按钮（-1 依赖 -一堆死键），除非英文用户是明确目标群体。**工作量 0.5d**
...
| 阶段 | 内容 | 工作量 | 里程碑 |
|---|---|---|---|
| **Phase 0（本批，已完成）** | 安全热修 S1-S9 + 第二/四批鉴权与加固 | 已完成 | 认证与数据暴露面收口 |
| **Phase 1** | §3.1 XSS 治理 + §3.2 构建瘦身 + §2.5 测试安全网 | ~12d | 安全闭环 + 重构安全网 |
| **Phase 2** | §2.1 lifespan/Alembic + §2.2 统一 ORM（先易后难） | ~10d | 数据层可控 |
| **Phase 3** | §2.3 worker 拆分 + §2.7 镜像化（含依赖升级） | ~6d | 部署可回滚 |
| **Phase 4** | §3.3 API 收口 + §3.4 死代码 + §3.5 TS 工具链 | ~13d | 前端工程化 |
| **Phase 5（渐进）** | §2.4 LLM Gateway + §2.6/2.8 + §3.6-3.8 | 按需 | 长期健康度 |
