<template>
  <div class="content-analysis data-dashboard">
    ...
    <!-- Phase 4c: 公共/我的 Tab 切换 -->
    <div class="stats-view-tabs">
      <button :class="['stats-view-tab',{active:statsTab==='public'}]" @click="statsTab='public'"><span>📊</span> 公共统计</button>
      <button :class="['stats-view-tab',{active:statsTab==='my'}]" @click="switchToMyStats"><span>📈</span> 我的统计</button>
    </div>

    <!-- Phase 4c: 我的统计面板 -->
    <div v-if="statsTab==='my'" class="my-stats-panel">
      <!-- 未登录提示 -->
      <el-card v-if="!isLoggedIn" shadow="never" class="my-stats-login-card">
        <div class="my-stats-login">
          <span class="my-stats-login-icon">🔐</span>
          <h3>登录后查看个人统计数据</h3>
          <p>查看您上传作品的题跋分析统计，与公共数据对比</p>
          <a href="/login" class="my-stats-login-btn">去登录</a>
        </div>
      </el-card>

      <!-- 已登录：加载中 -->
      <el-skeleton v-else-if="myStatsLoading" :rows="6" animated />

      <!-- 已登录：数据面板 -->
      <div v-else-if="myStatsData">
        <!-- 对比概览 -->
        <div class="stats-overview" style="margin-bottom:24px">
          <el-card shadow="hover" class="stat-card">
            <div class="stat-value">{{ myStatsData.total_count || 0 }}</div>
            <div class="stat-label">我的作品（幅）</div>
            <div class="stat-sub" style="font-size:11px;color:#999">占公共库 {{ myComparison?.my_percentage || 0 }}%</div>
          </el-card>
          <el-card shadow="hover" class="stat-card">
            <div class="stat-value">{{ myStatsPublicTotal || 0 }}</div>
            <div class="stat-label">公共作品总数</div>
            <div class="stat-sub" style="font-size:11px;color:#999">对比基准</div>
          </el-card>
        </div>

        <!-- 我的情感分布 -->
        <el-card v-if="myStatsData.sentiment_distribution?.length" shadow="hover" class="my-stats-card">
          <template #header><div class="card-header-title">我的情感分布</div></template>
          <div class="my-stats-table">
            <div class="my-stats-row head">
              <span>极性</span><span>作品数</span><span>占比</span>
            </div>
            <div class="my-stats-row" v-for="s in myStatsData.sentiment_distribution" :key="s.polarity">
              <span :style="{color:{positive:'#67c23a',negative:'#c04030',neutral:'#8b7d6b'}[s.polarity]||'#666'}">{{ {positive:'积极',negative:'消极',neutral:'中性'}[s.polarity]||s.polarity }}</span>
              <span>{{ s.count }}</span>
        ...
        <!-- 我的时期分布 -->
        <el-card v-if="myStatsData.period_stats?.length" shadow="hover" class="my-stats-card" style="margin-top:16px">
          <template #header><div class="card-header-title">我的时期分布</div></template>
          <div class="my-stats-table">
            <div class="my-stats-row head">
              <span>时期</span><span>作品数</span><span>均字数</span>
            </div>
            <div class="my-stats-row" v-for="p in myStatsData.period_stats" :key="p.period">
              <span>{{ p.period }}</span>
              <span>{{ p.count }}</span>
              <span>{{ p.avg_char_count }}</span>
            </div>
          </div>
        </el-card>

        <!-- 我的尺寸分布 -->
        <el-card v-if="myStatsData.size_distribution?.length" shadow="hover" class="my-stats-card" style="margin-top:16px">
          <template #header><div class="card-header-title">我的尺寸分布</div></template>
          <div class="my-stats-table">
            <div class="my-stats-row head">
              <span>类别</span><span>数量</span><span>占比</span>
            </div>
            <div class="my-stats-row" v-for="s in myStatsData.size_distribution" :key="s.category">
        ...
        <el-empty v-if="myStatsData.total_count===0" description="暂无个人作品数据，请先上传画作" :image-size="80" />
      </div>

      <!-- 加载失败 -->
      <el-empty v-else-if="myStatsError" description="加载个人统计失败" :image-size="80" />
    </div>

    <!-- Phase 4c: 公共统计面板（仅当 public tab 时显示） -->
    <div v-if="statsTab==='public'">
  ...
  </div><!-- End Phase 4c: public tab wrapper -->
...
// Phase 4c: 我的统计
const statsTab = ref('public')
const isLoggedIn = ref(!!localStorage.getItem('auth_token'))
const myStatsLoading = ref(false)
const myStatsError = ref(false)
const myStatsData = ref(null)
const myStatsPublicTotal = ref(0)
const myComparison = ref(null)

async function switchToMyStats() {
  statsTab.value = 'my'
  if (!isLoggedIn.value) return
  if (myStatsData.value) return // already loaded
  await loadMyStats()
}
...
async function loadMyStats() {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/tubi/stats/my`, {
    ...
    if (data.success) {
      myStatsData.value = data.my_stats
      myStatsPublicTotal.value = data.public_total
      myComparison.value = data.comparison
    } else {
