      ...
      <div class="ks-body-wrap" :class="{'with-panel':rightPanelOpen}">
        ...
        <transition name="slide-right"><div v-if="rightPanelOpen" class="ks-panel">
          ...
          <div class="ks-pbody">
            ...
            <div v-show="panelTab==='outline'" class="ks-ptab"><DocumentOutline :outline="documentOutline" :loading="loadingOutline" @item-click="onOutlineClick" /></div>
            ...
            <div v-show="panelTab==='images'" class="ks-ptab"><div class="ks-pimg-grid"><div v-for="(img,i) in activeResult?.associated_images" :key="i" class="ks-pimg-item" @click="openImagePreview(img)"><img :src="getImageUrl(img.url)" /><span v-if="img.figure_id">{{ img.figure_id }}</span></div></div></div>
          </div>
        </div></transition>
      </div>
    </div>
...
function getImageUrl(u){if(!u)return'';if(u.startsWith('http'))return u;return'/api/v1/knowledge/'+u.replace(/^\/+/,'')}
function getFullImageUrl(r){return getImageUrl(r.image?.url||r.associated_images?.[0]?.url)}
...
function onOutlineClick(item){if(item.title&&markdownContent.value){panelTab.value='markdown';nextTick(()=>{if(mdContentRef.value){const el=mdContentRef.value.querySelector(`[data-section-id="${item.id}"]`)||[...mdContentRef.value.querySelectorAll('h1,h2,h3,h4')].find(h=>h.textContent?.trim()===item.title?.trim());if(el)el.scrollIntoView({behavior:'smooth',block:'start'})}})};if(item.target_book_id&&item.page&&item.target_book_id!==activeResult.value?.book_id){const url=`/api/v1/knowledge/books/${item.target_book_id}/pdf#page=${item.page}`;window.open(url,'_blank')}else if(item.page&&pdfUrl.value&&!item.target_book_id){window.open(`${pdfUrl.value}#page=${item.page}`,'_blank')}}function onRelatedClick(c){if(c)activeResult.value=c}
async function sendChat(msg){const t=(msg||chatInput.value).trim();if(!t||chatLoading.value)return;if(!msg)chatInput.value='';chatMessages.value.push({role:'user',content:t});chatMessages.value.push({role:'assistant',content:'',thinking:true});chatLoading.value=true;try{const r=await fetch('/api/v1/knowledge/search',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query:t,limit:5})});const d=await r.json();chatMessages.value.pop();chatMessages.value.push({role:'assistant',content:d.ai_summary?.answer||'未找到相关信息',thinking:false})}catch{chatMessages.value.pop();chatMessages.value.push({role:'assistant',content:'查询失败，请重试',thinking:false})}finally{chatLoading.value=false;nextTick(()=>{if(chatMsgsRef.value)chatMsgsRef.value.scrollTop=chatMsgsRef.value.scrollHeight})}}
...
function openImagePreview(img){previewImageUrl.value=getImageUrl(img.url||img);previewVisible.value=true}
