...
/** Get unique map markers by merging locations at similar lat/lng */
export function getMergedLocations(): MergedLocation[] {
  ...
  for (const loc of LI_SHAN_LOCATIONS) {
    ...
    if (map.has(key)) {
      ...
      existing.periods.push(...loc.periods)
      existing.yearRanges.push(...loc.yearRanges)
      existing.description += '\n\n' + loc.description
      existing.sourceIds.push(loc.id)
    } else {
      map.set(key, {
        id: loc.id,
        name: loc.name,
