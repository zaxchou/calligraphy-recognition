...
from app.modules.pantianshou_composition.cv_preprocessor import run_cv_preprocess
...
1. **起必须是画材"大本营"与画面边缘的物理交汇点** —— "大本营"指画材的生物/物理起源点：植物的根部/鳞茎/盆底、器物的柄端/基座、山石的根基。起绝不是叶尖、花头、题跋或任意线条末端。**必须通过墨线走向、浓淡过渡、结构透视，逆推主干/主根的延长线，确认该延长线真实穿出画面边缘的位置。**
2. **起必须在画面边缘（x≤5 或 x≥95 或 y≤5 或 y≥95）且该位置有明确笔墨** —— 不在画面中央（x和y均在20-80间），不在留白处。起可以从画外开始，但交汇点必须落在边缘笔墨上。**严禁将"从右下边缘入画"误解为"起必须在右下角"；必须根据画材实际入画方向确定交汇点：若主干延长线从左边缘穿出，起就在左边缘；若从上边缘穿出，起就在上边缘。**
3. **起必须遵循绝对生长规律**：从根→干→枝→花/果/叶；从石基→枝干→花；从壶柄→壶身→壶嘴。起永远是生长链最前端，绝不可从花到根、从梢到基。
4. **起必须是单一生长链的唯一起点**：若画面有多个画材（如蒲扇+茶炉），只选择**有明确大本营且主干穿出边缘者**作为起的来源；其他画材不参与主轴线，除非它们与起同源（如一株梅树的多个枝干）。

**承（延伸）——固定且唯一**：
- **承必须是从起出发后，沿生长方向遇到的第一个、面积最大、笔墨最实的画材实体的面积中心点**（视觉重心，非边缘），且**必须与起属于同一画材、同一生长链**。
- **承与起必须空间邻近**：欧氏距离 ≤30%（画面宽高），且坐标变化必须符合生长习性：① 若起在底部（y≥90），承y值必须显著减小（向上生长）或略增（垂柳向下）；② 若起在顶部（y≤10），承y值必须增大；③ 若起在左侧（x≤10），承x值必须增大；④ 若起在右侧（x≥90），承x值必须减小。**承绝不可是另一画材的实体（如起是水仙鳞茎，承不能是茶炉）**。
- **承点必须在画材实体内部，不可在留白或边缘线上**。
...
- **转点坐标必须设在该视觉焦点的面积中心**，不可标在边缘或留白。

**合（收束）——固定且唯一，必须在画面内（x∈[5,95], y∈[5,95]）**：
- **合必须体现"回旋收束"趋势**：从转点到合点的向量，必须与从起到承的向量形成几何闭环（计算方法：设起→承向量为(vx,vy)，则转→合向量(wx,wy)需满足 |vx·wx + vy·wy| / (|v|·|w|) < cos(45°) ≈ 0.707，即夹角>45°且方向趋向闭合）。例如：起→承向左上，则转→合应向左下或右下。
- **题跋处理规则（必须按优先级执行）**：① 题跋占满一侧（如左竖长题），合取其纵向中点（x≈15, y=30±15）——**注意：y值必须偏下，因印章多在题跋下方，形成视觉重力链**；② 题跋在角落小块（如右上），合取其右下角附近（x=85, y=20），形成下沉收束；③ 题跋大面积贴边（顶/底且贴左/右），则忽略其构图作用，合改取印章群重心；④ **印章强化规则（强制应用）**：若印章在题跋下方/右侧，合点y/x坐标必须向印章坐标偏移至少10%；若印章独立成群（如右下两印），合取其群组中心；若印章在左上，合点x/y向左上偏移。
- **合绝不可是题跋几何中心的机械映射**，必须是题跋/印章群的**视觉重心**，且确保视线从转点自然"回落"至此。

**⚠️ 绝对禁止行为（违反任一即判定错误）**：
- 起不在边缘、起在留白、起在花头/叶尖/题跋；起未通过墨线延长线验证；
- 承不邻近起、承不在同一生长链、承在留白；承点y/x变化违背生长方向（如起在底部y=95，承y=85是合理向上，承y=90是无效微调，承y=95是错误）；
- 转不满足>45°方向突变、转脱离生长链、转在留白；转点非生长链末端；
- 合超出边界（x<5或x>95或y<5或y>95）、合无回旋趋势（未计算向量闭环）、合未响应印章位置；
- 将"生长方向描述"（如"从右下入画"）误当作"起必须在右下"；
- 忽略"大本营"概念，对多主干画材不追溯共同起源点；跨物类构建虚假生长链（如蒲扇→茶炉）；
- 承/转/合坐标未落在画材实体面积中心（如标在叶脉线上、花蕊点、印章边框）。

**分析步骤（必须严格按序执行，每步需验证）**：
1. **识别大本营并验证起**：观察所有画材墨线走向，逆推主干/主根延长线，确认其与画面边缘（x≤5/x≥95/y≤5/y≥95）的交汇点；检查该交汇点是否有笔墨；确认其为生长链最前端（根/鳞茎/基座）。若多交汇点，选墨线最粗、最实、延伸感最强者。
2. **定位承**：从起出发，沿生长方向（根→干→枝）移动，找到第一个面积最大、笔墨最实的**同一画材实体**，取其中心点；验证：① 欧氏距离≤30%；② 坐标变化符合生长方向（如起y=95→承y≤65）；③ 实体内部有笔墨。
3. **定位转**：从承继续沿同一生长链前进，找到首个方向突变（>45°）且视觉张力最强的**链末端节点**（分叉/花头/鸟首），取其中心点；验证：① 向量夹角>45°；② 在生长链末端；③ 实体内部有笔墨。
4. **定位合**：① 判断题跋类型及印章位置；② 按题跋规则初定合点；③ 强制应用印章牵引（向印章坐标偏移≥10%）；④ 计算转→合向量与起→承向量的夹角，确保>45°且趋向闭环；⑤ 验证x,y∈[5,95]。
5. **全局验证**：用一根虚拟笔触连接起→承→转→合，确认：① 全程不提笔（四点连线不穿越大片留白）；② 不逆向（y/x单调性符合生长方向）；③ 四点共构一条平滑曲线（S/Z/三角等）；④ 所有点均在画材实体面积中心，非边缘或留白。
...
def run_phase1_collect(max_samples=None):
    ...
    for i, fname in enumerate(files):
        ...
        try:
            ...
            cv_result = run_cv_preprocess(img)
