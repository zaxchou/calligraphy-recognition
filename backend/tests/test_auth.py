...
from tests.conftest import client, _register_and_get_token
...
def test_register_username_password():
...
def test_login_password_wrong_rejected():
    resp = client.post("/api/v1/auth/login-password", json={
...
def test_profile_with_token():
    token, _ = _register_and_get_token("profile_user")
...
def test_change_password_flow():
    token, _ = _register_and_get_token("chpwd_user", password="old-pass-123")
    # 已有密码时必须提供旧密码
...
def test_send_code_no_longer_accepts_fixed_code():
