...
# DATA_DIR: 所有存储到 DB 的路径都是相对于此目录的相对路径
# 在模块加载时计算，避免循环导入
_DATA_DIR = os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'data'))
...
def normalize_path(path: str) -> str:
    ...
    将路径标准化，统一使用正斜杠 '/'
    用于存储到数据库的路径
    绝对路径自动转为相对于 DATA_DIR 的相对路径（如 data/thumbnails/xxx.jpg）
    """
    if not path:
        return path
    # 统一正斜杠
    normalized = path.replace('\\', '/')
    # 如果是绝对路径，尝试转为相对于 _DATA_DIR 的相对路径
    if normalized.startswith('/') or (len(normalized) > 1 and normalized[1] == ':'):
        # 去掉常见前缀，保留 data/ 后面的相对路径
        # Z:/BaiduSync/.../backend/data/uploads/xxx.jpg -> data/uploads/xxx.jpg
        # /foo/bar/data/uploads/xxx.jpg -> data/uploads/xxx.jpg
        parts = normalized.replace('\\', '/').split('/')
        # 找到 'data' 在路径中的位置，保留其后的部分
        try:
            data_idx = parts.index('data')
            relative = '/'.join(parts[data_idx:])
            # 验证转换后的相对路径是合理的（以 data/ 开头）
