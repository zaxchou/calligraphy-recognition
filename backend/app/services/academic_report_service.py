...
ARTIST_CONFIG = {
    "李鱓": {
        ...
        "defense_qa": [
            {
                "question": "消极占比55.1%是否说明李鱓一贯消极？",
                "answer": "李鱓情感并非'一贯消极'。整体消极55.1%（226/410件）反映其题跋中批评、愤懑、自伤内容的确实占多数，但仍有32.2%积极作品。需注意同一题跋可能同时包含积极自我期许与消极现实感慨，情感结构是复合的而非单一标签。",
            },
            {
                "question": "咏物寄兴占77.6%是否说明分类器缺乏区分度？",
                "answer": "咏物寄兴是文人画题跋的本体功能，占比高是预期现象。李鱓画松竹梅菊以寓品格、画蔬果以见性情，正是其创作核心。一件作品可同时归入多个主题（如既是咏物又含交游），因此各主题之和超过100%。故宫《全集》前言亦称其'每有所作必题诗其上，借物抒怀'。",
            },
            {
                "question": "样本是否偏向传世名作而影响代表性？",
                "answer": "本研究收录李鱓传世有题跋作品410件，基本覆盖海内外公私收藏中可见的李鱓题跋画作。83.9%的作品有明确纪年，数据完整性较高。传世作品存在'幸存者偏差'（精品更易保存），但410件的样本规模在该领域已属前列。",
            },
            {
                "question": "AI辅助分析的可信度如何？",
                "answer": "采用双通道验证机制：规则引擎（v5.6）作为主通道，大语言模型作为低可信度场景的二次复核。当规则引擎置信度低于阈值时自动触发AI复核。分析结果在'情感倾向'和'主题分类'两个维度均保留完整的推导步骤（reasoning_steps），可供逐条审验。",
            },
            {
                "question": "题跋分期依据是否可靠？",
                "answer": "分期方案综合三方面证据：一是明确纪年作品（344/410，占83.9%）；二是文献可考的生平大事（入宫供奉、罢官南归等时间节点）；三是风格断代与印章演化（如'懊道人'号的使用时期）。无明确年代依据者标记为'年代不详'单独统计，不参与时期对比。",
            },
            {
                "question": "身世自况与咏物寄兴是否存在边界模糊？",
                "answer": "两者确实存在过渡地带。区分原则：以物象为论述中心、情感不指向作者自身者归为咏物寄兴；明确出现'余'、'予'、'老夫'等第一人称且表达身世之感的归为身世自况。当两类特征同时出现时，以主导倾向判定。",
            },
        ],
        "conclusion_points": [
            "创作模式：咏物寄兴为最核心主题（77.6%）。李鱓是典型的'以画入诗'型文人画家，绝大多数题跋围绕画面物象展开。",
            "情感底色：整体情感偏负面（均值-0.77，消极占55.1%）。但并非'一贯消极'——32.2%作品呈现积极情感，情感结构是复合的。",
            "身份表达：身世自况覆盖67.8%作品。'落拓''潦倒'等自况语言高频出现，反映李鱓将题跋作为自我叙述的重要载体。",
            "社交属性：交游赠答与吉语祥瑞分别覆盖29.5%和26.1%。作为扬州画派核心成员，应酬性创作占相当比例，与美术史对其'职业画家'的定位一致。",
        ],
    },
    "郑燮": {
        ...
        "defense_qa": [
            {
                ...
                "answer": "郑燮以兰竹见长，题跋多借物抒怀与画理自叙，数据分布应与此特征吻合。郑板桥自述'画兰画竹皆画人'，其题跋本身即是人格表达。",
            },
            {
                "question": "郑燮与李鱓同为扬州八怪，题跋风格有何异同？",
                "answer": "李鱓题跋情感波动更大、消极色彩更浓（晚期61.7%消极），与其它屐后潦倒经历直接相关。郑燮虽亦仕途坎坷，但其题跋中'难得糊涂'式的超脱多于愤怒，情感强度整体低于李鱓。两者共同点是咏物寄兴均为最大主题类别。",
            },
            {
                "question": "郑燮的'诗书画三绝'在数据中如何体现？",
                "answer": "郑板桥题跋常将诗与画紧密结合，题画诗内容往往独立成篇。数据中画理自叙类占比体现其自觉的艺术理论思考——他是有意识地将创作理念写入题跋的代表性画家，这与美术史评价一致。",
            },
        ],
        "conclusion_points": [],
    },
...
def _build_markdown(stats: Dict, artist_cfg: Dict, artist: str) -> str:
    ...
    w(f"3. **可信度**：高可信度占 {_fmt_pct(confidence_dist['high'], total)}（≥0.70），中可信度 {_fmt_pct(confidence_dist['medium'], total)}（0.40–0.69），低可信度 {_fmt_pct(confidence_dist['low'], total)}（<0.40）。")
    w()
    ...
    if low_cases:
        w("### 5.3 低可信度边界案例（需人工复核）")
        for i, case in enumerate(low_cases[:8], 1):
            w(f"**案例{i}**：{case['title']}（{case['period']}，conf={case['confidence']:.2f}）")
            w(f"- 主题：**{case['theme']}** | 情感：{case['polarity']}（{case['emotion_score']:+.2f}）")
            w(f"- 题跋：*{case['text']}*")
            w()
        w("> 低可信度成因：（1）题跋过短；（2）多主题得分接近；（3）内容模糊/残损。")
        w()
...
def _build_sections(stats: Dict, artist_cfg: Dict, artist: str) -> List[Dict]:
    ...
    abstract_lines.append(
        f"3. **可信度**：高可信度占 {_fmt_pct(confidence_dist['high'], total)}（≥0.70），中可信度 {_fmt_pct(confidence_dist['medium'], total)}（0.40–0.69），低可信度 {_fmt_pct(confidence_dist['low'], total)}（<0.40）。"
    )
    ...
    sections.append({"id": "confidence", "title": "可信度分析", "type": "table", "content": conf_table})
    ...
    if high_cases:
        ...
        sections.append({"id": "high_confidence", "title": "高可信度案例", "type": "list", "content": cases})
    ...
    if low_cases:
        ...
        sections.append({"id": "low_confidence", "title": "低可信度边界案例", "type": "list", "content": cases})
