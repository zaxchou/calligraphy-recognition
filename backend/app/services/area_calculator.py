...
def calculate_area_stats_fillpoly(
    ...
    优先级：余边(4) > 题跋(1) > 绘画(2) > 留白(3)，按留白→绘画→题跋→余边的顺序 fillPoly，
    后绘制的覆盖先绘制的，自然实现优先级。
    ...
    # mask 值: 0=未分类, 1=题跋, 2=绘画, 3=留白, 4=余边
    # 按优先级从低到高绘制：留白→绘画→题跋→余边，高优先级覆盖低优先级
    ...
    margin_px = int(np.count_nonzero(mask == 4))
    # 未被任何区域覆盖的像素也归为留白
    ...
    margin_percent = (margin_px / total_area * 100.0) if total_area > 0 else 0.0
    blank_percent = 100.0 - inscription_percent - painting_percent - margin_percent
    ...
    return {
        ...
        "margin_area": round(total_area * margin_percent / 100.0, 2),
...
def calculate_pixel_based_area_stats_exclusive(
    ...
    margin_polys = [region_to_polygon(r) for r in regions.get("margin_regions", [])]
    ...
    margin_polys = [p for p in margin_polys if len(p) >= 3]
    ...
    for y in range(0, image_height, sample_step):
        for x in range(0, image_width, sample_step):
            ...
            # 按优先级检查点是否在区域内
            # 优先级0：余边（最高）
            in_margin = any(point_in_polygon(point, poly) for poly in margin_polys)
            if in_margin:
                margin_pixels += 1
                continue
    ...
    # 计算百分比
    if total_sampled > 0:
        margin_percent = (margin_pixels / total_sampled) * 100
    ...
    # 严格确保总和为100%
    total_percent = margin_percent + inscription_percent + painting_percent + blank_percent
    if abs(total_percent - 100) > 0.001:
        # 将差值加到留白上（留白优先级最低，作为兜底）
        blank_percent += (100 - total_percent)

    # 最终校验
    final_total = margin_percent + inscription_percent + painting_percent + blank_percent
    if abs(final_total - 100) > 0.01:
        # 如果还有误差，按比例调整
        scale = 100 / final_total
        margin_percent *= scale
    ...
    margin_area = total_area * (margin_percent / 100)
    ...
    return {
        "total_area": total_area,
        "margin_area": round(margin_area, 2),
        ...
        "region_counts": {
            "margin_regions": len(margin_polys),
...
def calculate_pixel_based_area_stats(
    ...
    return {
        "total_area": total_area,
        "margin_area": margin_pixels * pixel_area,
