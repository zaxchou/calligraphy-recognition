...
def _get_artist_sentiment_note(artist: str = None) -> str:
    ...
    if name == "李鱓":
        return ("李鱓别号'懊道人'，情感底色偏阴。题跋多含蓄，'借景抒怀'常暗藏压抑愤懑。"
                "**表面写景但有愁苦词即判negative**。"
                "含'懊道人'署名的吉语题跋可能是反讽，倾向判negative。")
    return ""
...
def _get_artist_theme_note(artist: str = None) -> Tuple[str, str]:
    ...
    if name == "李鱓":
        return (
            "- ⚠️ 李鱓核心主题是'身世自况'：凡是涉及自身境遇（仕途、卖画、困顿、自嘲）的题跋，"
            "都应归为'身世自况'而非'咏物寄兴'或'吉语祥瑞'。"
            "含'懊道人'署名的题跋，即使有吉祥词（如'加官''大吉'等），仍可能是身世自况或反讽，"
            "需结合全文判断。'老夫卖画''臣非老画师'等是身世自况的强信号。",
            "、懊道人、复堂"
        )
    return ("", "")
...
# ══════════════════════════════════════════════════════════════════════
# 六大主题定义 —— 意图导向分类（v5 重构版）
# ══════════════════════════════════════════════════════════════════════
# 编码保持不变（1-6），名称和定义按"核心意图"重新设计
# 关键变化：
#   - "记录创作信息"降级，不再是独立主题（文人画无"纯记录"）
#   - "身世自况"独立为第一类，是李鱓最核心的主题
#   - "咏物寄兴"取代"即景寄兴"，强调"借物"而非"即景"
#   - "吉语祥瑞"取代"世俗祈愿"，更准确
#
# 主题代码映射（前端兼容，编码不变）：
#   1=身世自况  2=咏物寄兴  3=画理自叙
#   4=时事讽喻  5=吉语祥瑞  6=交游赠答
# ══════════════════════════════════════════════════════════════════════

THEMES = {
    1: {
        "name": "身世自况",
        "description": "借画/题跋映射自身境遇、身份认同、人生感慨。李鱓标志性主题。",
        "keywords": [
            "懊道人", "复堂", "臣非老画师", "两革科名", "一贬官", "老夫卖画",
            "落拓", "潦倒", "落魄", "穷愁", "困顿", "失意", "遭际", "坎坷",
            "科名", "罢官", "仕途", "官场", "浮沉", "升沉",
            "白发", "衰年", "老病", "年老", "迟暮", "暮年",
            "艰难", "困苦", "衣食", "卖画", "砚田",
            "故山", "归隐", "归田", "还乡", "归",
    ...
    2: {
        "name": "咏物寄兴",
        "description": "借所画之物抒发情志、寄托品格理想。传统文人画核心功能。",
        "keywords": [
            "松", "竹", "梅", "兰", "菊", "岁寒三友", "四君子",
            "傲骨", "凌霜", "劲节", "幽香", "孤高", "清逸", "淡泊",
            "苍松", "劲竹", "傲霜", "凌云", "清芬", "高节",
            "隐逸", "高士", "幽人", "幽居", "闲居",
            "清风", "明月", "幽", "远", "静", "逸",
        ],
    ...
    3: {
        "name": "画理自叙",
        "description": "阐述笔墨技法、宗派师承、创作理念、风格追求。",
        "keywords": [
            "笔法", "墨法", "画理", "师承", "宗派", "门户",
            "仿", "拟", "摹", "临", "笔意", "墨意",
            "用笔", "用墨", "画法", "技法", "泼墨", "写意",
    ...
    4: {
        "name": "时事讽喻",
        "description": "批判社会现实、关怀民生疾苦、讽刺权贵世态。",
        "keywords": [
            "催租", "舆隶", "官粮", "纨绔", "夺朱", "恶态",
            "世味辣", "卖画难", "官吏", "赋税", "民生", "苍生",
            "豪门", "簪缨", "冷落", "俗尘", "市井", "势利",
            "挥毫卖", "寻纸", "索画", "催诗", "催画",
            "艰难", "恼客魂", "衣食", "画贱", "何苦", "味犹苦", "百结衣",
            "苦", "寒", "难", "恼", "愁", "辣", "悔", "怨", "愤",
        ],
    },
    5: {
        "name": "吉语祥瑞",
        "description": "吉祥祝福、谐音寓意（鸡=吉、鱼=余）、生活化谐趣。",
        "keywords": [
            "大吉", "百事大吉", "多寿", "子孙贤", "平安", "如意",
            "富贵", "荣华", "福", "寿", "吉", "顺遂", "同到白头",
            "加官", "长命", "吉祥", "祝福", "百子", "长年",
            "喜", "笑", "戏", "娱", "玩", "谐", "趣",
        ],
    ...
    6: {
        "name": "交游赠答",
        "description": "为亲友/官员所作，含祝贺、送别、应酬、请正。",
        "keywords": [
            "请指教", "敬请", "雅正", "指正", "惠存", "存念", "补壁",
            "赠", "送", "奉赠", "为", "作", "写似",
            "祝", "贺", "寿", "庆", "喜",
            "年长兄", "仁兄", "亲翁", "先生", "居士",
            "雅属", "鉴", "乞教", "奉",
...
# ── 表2：题材-主题/情感关联表（v5 重构版）────────────────────────────────
# 画材关键词 → 视觉情感 → 默认主题倾向 + 证据权重
# 关键变化：
#   - 四君子 → 咏物寄兴（强调品格寄托，不只是"即景"）
#   - 蔬果 → 咏物寄兴（李鱓"取材之广"是文人特质，非世俗谐趣）
#   - 墨荷/风雨/枯木 → 身世自况（萧瑟意象映射境遇）
#   - 牡丹/吉祥 → 吉语祥瑞
PAINTING_MATERIAL_RULES = [
    {
        "keywords": ["松", "梅", "兰", "竹", "菊", "岁寒三友", "四君子",
                     # 题跋中常见的高级表达（v5.4 扩展）
                     "苍松", "虬松", "乔松", "老松", "劲松", "孤松", "松风", "松涛",
                     "修竹", "翠竹", "丛竹", "竹石", "新篁", "幽篁", "筠", "竿",
                     "寒梅", "瘦梅", "墨梅", "红梅", "早梅", "暗香", "疏影", "冰肌",
                     "幽兰", "空谷兰", "兰草", "纫兰",
                     "秋菊", "黄菊", "东篱", "霜菊", "傲霜",
                     "劲节", "凌霜", "风霜"],
        "visual_emotion": "傲骨/坚韧",
        "theme_tendency": 2,   # 咏物寄兴
        "emotion_offset": 0.0, # 中性偏积极（傲骨非消极）
        "weight": 1.2,
        "description": "四君子/岁寒三友 → 咏物寄兴（品格寄托）",
    },
    {
        "keywords": ["墨荷", "风雨", "枯木", "残荷", "芭蕉", "萧瑟", "败叶",
                     # v5.4 扩展：更多萧瑟意象
                     "枯藤", "老树", "昏鸦", "寒鸦", "孤鸟", "独鹤",
                     "荒寒", "荒寂", "寥落", "萧条", "零落", "凋残",
                     "菡萏", "芙蕖", "红蕖", "白莲", "莲花", "并蒂莲", "出淤泥",
                     "绿天", "蕉窗", "雨打芭蕉"],
        "visual_emotion": "萧瑟/压抑",
        "theme_tendency": 1,   # 身世自况（萧瑟意象映射境遇）
        "emotion_offset": -0.5,
        "weight": 1.5,
        "description": "墨荷/风雨/枯木 → 身世自况（借萧瑟写心境）",
    },
    {
        "keywords": ["牡丹", "藤蔓", "松藤", "松石", "百子", "富贵", "长年", "松鹤",
                     # v5.4 扩展：更多富贵吉祥意象
                     "国色", "天香", "魏紫", "姚黄", "洛阳花", "花王",
                     "仙桃", "蟠桃", "寿桃", "灵芝", "瑞草", "祥云",
                     "紫藤", "紫气", "紫气东来", "凌霄", "凌霄花"],
        "visual_emotion": "富贵/长寿",
        "theme_tendency": 5,   # 吉语祥瑞
        "emotion_offset": 0.5,
        "weight": 1.3,
        "description": "牡丹/松鹤/富贵 → 吉语祥瑞",
    },
    {
        "keywords": ["蔬果", "葱", "蒜", "姜", "白菜", "萝卜", "茄", "瓜", "茭白", "山芋", "杂画",
                     # v5.4 扩展：更多日常蔬果
                     "南瓜", "丝瓜", "冬瓜", "葫芦", "扁豆", "豌豆", "豆荚",
                     "芋头", "荸荠", "菱角", "藕", "笋", "竹笋", "蘑菇",
                     "芹菜", "韭菜", "莴苣", "蔓菁", "芜菁", "蕨菜", "荠菜",
                     "石榴", "枇杷", "荔枝", "龙眼", "樱桃", "杨梅", "柑橘"],
        "visual_emotion": "生活化/质朴",
        "theme_tendency": 2,   # 咏物寄兴（李鱓"取材之广"是文人特质）
        "emotion_offset": 0.0,
        "weight": 1.0,
        "description": "蔬果 → 咏物寄兴（日常物中见性情）",
    },
    {
        "keywords": ["鸡", "柏", "鱼", "柿", "加官", "大吉", "喜上眉梢", "百事大吉",
                     # v5.4 扩展：更多谐音吉祥
                     "雄鸡", "锦鸡", "金鸡", "报晓", "冠", "官上加官",
                     "鲤鱼", "金鱼", "锦鲤", "双鱼", "年年有余",
                     "石榴", "多子", "百子图", "榴开百子",
        ...
        "emotion_offset": 0.5,
        "weight": 1.4,
        "description": "鸡柏鱼柿 → 吉语祥瑞（谐音寓意）",
    },
    {
        "keywords": ["鸟", "雀", "燕", "鹤", "鹭", "鹦鹉",
                     # v5.4 扩展：更多禽鸟
                     "翠鸟", "黄鹂", "鸳鸯", "大雁", "鸿雁", "白鹭", "白鹭鸶",
                     "八哥", "画眉", "白头翁", "绶带", "锦鸡", "孔雀",
                     "鹤寿", "松鹤", "仙鹤", "丹顶鹤", "鹤立", "鹤鸣",
                     "鱼鹰", "鸬鹚", "水禽", "水鸟"],
        "visual_emotion": "灵动/自在",
        "theme_tendency": 2,   # 咏物寄兴
        "emotion_offset": 0.2,
        "weight": 0.8,
        "description": "禽鸟 → 咏物寄兴（自在/孤高）",
    },
    # v5.4 新增规则：鱼虾蟹 → 咏物寄兴
    {
        "keywords": ["鱼", "虾", "蟹", "鲤鱼", "金鱼", "锦鲤", "游鱼", "双鱼",
                     "青虾", "游虾", "虾趣", "大虾", "湖虾",
                     "螃蟹", "闸蟹", "毛蟹", "河蟹", "蟹肥", "蟹正肥"],
        "visual_emotion": "鲜活/自在",
        "theme_tendency": 2,   # 咏物寄兴
        "emotion_offset": 0.0,
        "weight": 1.0,
        "description": "鱼虾蟹 → 咏物寄兴（水族生机）",
    },
    # v5.4 新增规则：葡萄/紫藤/凌霄 → 咏物寄兴（藤蔓植物，李鱓常画）
    {
        "keywords": ["葡萄", "紫珠", "马乳", "龙须", "明珠", "珠圆",
                     "紫藤", "藤花", "藤萝", "老藤", "枯藤", "藤蔓"],
        "visual_emotion": "繁茂/珠玉",
        "theme_tendency": 2,   # 咏物寄兴
        "emotion_offset": 0.0,
        "weight": 1.0,
        "description": "葡萄/紫藤 → 咏物寄兴（珠玉满架）",
    },
    # v5.4 新增规则：山水泉石 → 画理自叙/咏物寄兴（视题跋内容而定）
    {
        "keywords": ["山水", "泉石", "云壑", "溪流", "飞瀑", "瀑布",
                     "层峦", "峰峦", "烟岚", "烟霞", "丘壑", "溪山",
                     "松石", "松石图", "松石图轴"],
        "visual_emotion": "高远/清幽",
        "theme_tendency": 2,   # 默认咏物寄兴（文人山水情怀）
        "emotion_offset": 0.0,
        "weight": 0.8,
        "description": "山水泉石 → 咏物寄兴（隐逸情怀）",
    },
]

# ── 表3：文本特征词库（权重打分表）v5 重构版 ────────────────────────────
# 主题代码: 1=身世自况, 2=咏物寄兴, 3=画理自叙,
#           4=时事讽喻, 5=吉语祥瑞, 6=交游赠答
#
# 设计原则：
#   - 身世自况权重最高（李鱓核心），关键词覆盖其标志性意象
#   - 去掉"记录创作信息"的base_score=1和兜底逻辑
#   - 每个主题有明显区分度，减少交叉误判
# ═══════════════════════════════════════════════════════════════════════

TEXT_SCORING_RULES = {
    # ── 1. 身世自况 ───────────────────────────────────────────────────
    # 李鱓最核心的主题。凡是涉及自身境遇、身份感慨、人生失意的内容归此类。
    # v5.1 迭代校准：降低弱信号权重，避免吸走其他主题
    1: {
        "keywords": {
            # +3分 — 李鱓标志性身份/境遇词（强信号，一旦触发直接竞争第一主题）
            # v5.3: 移除"复堂""懊道人"——二者在绝大多数作品中仅为落款署名，
            #       与题跋内容主题无必然关联，纳入评分会导致签名形式系统性干扰内容分类。
            #       保留"科名"等真正反映身份境遇的词。
            "臣非老画师": 3,
            "两革科名": 3, "一贬官": 3, "老夫卖画": 3,
            "落拓": 3, "潦倒": 3, "落魄": 3, "穷愁": 3,
            "科名": 3, "罢官": 3, "仕途": 3, "浮沉": 3, "升沉": 3,
            # +2分 — 个人境遇/心境词（中信号）
            "困顿": 2, "失意": 2, "遭际": 2, "坎坷": 2,
            "白发": 2, "衰年": 2, "老病": 2, "迟暮": 2, "暮年": 2,
            "艰难": 2, "困苦": 2, "衣食": 2, "卖画": 2, "砚田": 2,
            "故山": 2, "归隐": 2, "归田": 2, "还乡": 2,
            "孤": 2, "愁": 2, "凄": 2, "寂": 2, "惘": 2,
            "何用": 2, "何意": 2, "不堪": 2, "无复": 2,
            "空山": 2, "空庭": 2, "萧疏": 2, "萧瑟": 2,
            # v5.2: 弱信号权重进一步降到0.3，移除归/隐/逸（这些更属于咏物寄兴）
            "老": 0.3, "穷": 0.3, "苦": 0.3, "寒": 0.3, "闷": 0.3,
            "残": 0.3, "衰": 0.3, "败": 0.3, "倦": 0.3, "遥": 0.3,
            "叹": 0.3, "慨": 0.3, "惆": 0.3, "悔": 0.3, "恨": 0.3,
        },
        "priority_override": True,  # 得分>=3时具有强竞争力
        "override_threshold": 3,
        "overrides_theme": 2,       # 覆盖咏物寄兴
    },

    # ── 2. 咏物寄兴 ───────────────────────────────────────────────────
    # 借所画之物抒发情志、寄托品格。关键词侧重"物→品格"的映射。
    2: {
        "keywords": {
            # +3分 — 四君子+品格词（强信号）
            "岁寒三友": 3, "四君子": 3,
            ...
            "孤高": 3, "清逸": 3, "淡泊": 3, "高洁": 3,
            # +2分 — 具体物象+品格关联
            "苍松": 2, "劲竹": 2, "傲霜": 2, "凌云": 2, "清芬": 2, "高节": 2,
            "隐逸": 2, "幽人": 2, "幽居": 2, "闲居": 2,
            "松": 2, "竹": 2, "梅": 2, "兰": 2, "菊": 2,
            # +1分 — 氛围/意境词
            "清风": 1, "明月": 1, "幽": 1, "远": 1, "静": 1, "逸": 1,
            "闲": 1, "雅": 1, "淡": 1, "清": 1,
            "观": 1, "赏": 1, "爱": 1, "寄": 1, "兴": 1,
            "山水": 1, "泉": 1, "溪": 1, "云": 1, "鹤": 1,
            # v5.2: 从身世自况移入——归隐/隐逸更属于咏物寄兴（品格表达）
            "归隐": 2, "归田": 2, "还乡": 2, "隐逸": 2, "幽居": 2, "闲居": 2,
            "归": 1, "隐": 1,
        },
    ...
    3: {
        "keywords": {
            # +3分 — 技法/师承核心词
            "笔法": 3, "墨法": 3, "画理": 3, "师承": 3, "宗派": 3,
            "写意": 3, "泼墨": 3, "我法": 3, "自成": 3,
            "仿": 3, "拟": 3,  # v5.1: 从2提升到3，"仿XX笔意"是强信号
            # +2分 — 创作行为+理论词
            "笔意": 2, "墨意": 2,
            "用笔": 2, "用墨": 2, "画法": 2, "技法": 2,
            "水墨": 2, "笔墨": 2, "设色": 2, "工细": 2,
    ...
    # ── 4. 时事讽喻 ───────────────────────────────────────────────────
    # 社会批判、民生关怀。与身世自况的区别：侧重"社会/他人"而非"个人"。
    # v5.1: 增加更多中等信号词，提升竞争力
    4: {
        "keywords": {
            # +3分 — 强批判词（直接锁定）
            "催租": 3, "舆隶": 3, "官粮": 3, "纨绔": 3, "夺朱": 3,
            "世味辣": 3, "卖画难": 3, "官吏": 3, "赋税": 3,
            "纨绔子弟": 3, "恶态": 3, "画贱": 3, "百结衣": 3,
            # +2分 — 社会/批判词
            "苍生": 2, "民生": 2, "民": 2, "吏": 2,
            "租": 2, "官": 2, "世间": 2, "天下": 2,
            "世味": 2, "豪家": 2, "冷落": 2, "簪缨": 2,
            ...
            "艰难": 2, "恼客魂": 2, "衣食": 2, "何苦": 2, "味犹苦": 2,
            # v5.1: 增加更多社会批判中等信号词
            "卖画": 2, "利市": 2, "不得": 2, "没处": 2,
            "权": 2, "霸": 2, "富": 2, "贵": 2,
            # +1分 — 弱批判/社会词（需组合）
            "富贵": 1, "名利": 1, "荣华": 1,  # 在批判语境下
        },
        "priority_override": True,  # 得分>=2时直接锁定
    ...
    6: {
        "keywords": {
            # +3分 — 明确赠送/应酬词
            "奉赠": 3, "写似": 3, "雅正": 3, "惠存": 3, "补壁": 3,
            # +2分 — 社交行为词
            "祝": 2, "贺": 2, "赠": 2, "送": 2, "为": 2,
            "作": 2, "写": 2, "画": 2,  # 在"为XX"语境下
            "敬请": 2, "指正": 2, "存念": 2,
            "雅属": 2, "鉴": 2, "乞教": 2, "奉": 2,
            "年长兄": 2, "仁兄": 2, "亲翁": 2,
            # +1分 — 弱社交词
        ...
        "priority_override": False,
        "requires_recipient": True,  # 需要明确受赠人才高置信度
    },
}
...
# 设计原则：
#   - 李鱓整体情感基线偏阴（"懊"字当头）
#   - 消极词权重 > 积极词权重（避免被花鸟画题材表面"雅"误导）
#   - 增加"自嘲"类别（李鱓特有：懊道人、臣非老画师等）
#   - 区分"强消极"（社会批判）和"身世消极"（个人感慨）
...
# ── 画家情感基线修正 ──────────────────────────────────────────────────
# 不同画家有不同的情感"底色"，用于全局修正 emotion_score
ARTIST_EMOTION_BASELINE = {
    "李鱓": -0.5,      # "懊道人"底色偏阴
    "郑燮": -0.3,      # 郑板桥也有牢骚
    "金农": -0.2,      # 冬心先生偏孤冷
    "黄慎": -0.1,      # 瘿瓢相对中性
...
THEME_SENTIMENT_OVERRIDE = {
    1: {  # 身世自况 → 强制偏阴（除非有明确吉祥词对冲）
        "polarity": "negative",
        "min_score": -1.5,
        "override_bonus": -1.0,
        "note": "身世自况主题默认偏消极（人生失意/自嘲底色）",
        "exception_keywords": ["大吉", "平安", "多寿", "子孙贤"],  # 有这些词时允许positive
    },
    4: {  # 时事讽喻 → 强制消极
        "polarity": "negative",
        "min_score": -1.0,
    ...
    5: {  # 吉语祥瑞 → 允许积极（但李鱓需警惕"假吉祥真讽喻"）
        "polarity": "positive",
        "min_score": 0.5,
        "override_bonus": 1.0,
        "note": "吉语祥瑞主题倾向积极",
        # 李鱓的特殊规则：含"懊道人"署名的吉语题跋，可能是反讽
        "artist_exception": {
            "李鱓": {
                "override_if_contains": ["懊道人", "世味", "辣", "苦"],
                "override_to": "negative",
            }
        },
    },
}
...
# 尺寸×题材→主题权重（v5 重构版，主题编码已更新）
SIZE_THEME_RULES = [
    {
        "condition": {"size": "大幅", "materials": ["吉祥", "牡丹", "松柏", "祝寿", "富贵", "长寿"]},
        "theme_boost": {6: 1.5},  # 交游赠答 +1.5
        "description": "大幅+吉祥题材→交游赠答/祝寿/厅堂装饰",
    },
    {
        "condition": {"size": "小幅", "materials": ["四君子", "墨荷", "山水"]},
        "theme_boost": {2: 1.0},  # 咏物寄兴 +1.0
        "description": "小幅+四君子/墨荷→案头清玩/咏物寄兴",
    },
    {
        "condition": {"size": "小幅", "materials": ["墨荷", "残荷", "枯木"]},
        "theme_boost": {1: 1.0},  # 身世自况 +1.0
        "description": "小幅+萧瑟意象→身世自况（借景写心）",
    },
    {
        "condition": {"size": "中幅", "materials": ["鱼虾", "蔬果"]},
        "theme_boost": {2: 0.5},  # 咏物寄兴 +0.5
        "description": "中幅+鱼虾/蔬果→咏物寄兴（日常物中见性情）",
    },
]
...
SIZE_PERIOD_MOOD_RULES = [
    {
        ...
        "theme_boost": {3: 1.0},  # 画理自叙 +1.0（进呈/正式创作，注重技法展示）
        "sentiment_modifier": 0.2,
        "description": "早期大幅→进呈之作，风格工致",
    },
    {
        "condition": {"size": "大幅", "period": "中期"},
        "theme_boost": {4: 0.5},  # 时事讽喻 +0.5（职业画家需要关注）
        "sentiment_modifier": -0.2,
    ...
    },
    {
        ...
        "theme_boost": {1: 0.5},  # 身世自况 +0.5
        "sentiment_modifier": -0.3,  # 归隐身心态
        "description": "晚期小幅→归隐后墨戏，身世自况",
        "mood_tag": "归隐自遣",
    },
]
...
def classify_inscription_v4(
    ...
    if baseline != 0.0:
        if abs(text_emotion_score) < 1.5:
            # v5.2: 修正幅度从0.5降到0.3；早期作品再减半
            baseline_factor = 0.3
            if life_stage.get("stage", "").startswith("早期"):
...
- 自嘲自况：以"懊道人"署名、自称"臣非老画师"、感慨"两革科名一贬官"
- 关键词：懊、老夫、白发、艰难、困、寒、凄、恼、恨、俗尘、催租、画贱、世味、辣、悔、孤、愁、遥、落拓、潦倒

**neutral（中性）**：纯技法讨论、无明显情感表达

【特别注意】
{artist_note}
- ⚠️ **"懊道人"署名是自嘲标志**：即使题跋表面有吉祥词（如"加官""大吉"），署名"懊道人"暗示整首题跋可能是反讽/自嘲，倾向判negative
...
# 组合分析 Prompt：一次调用同时返回主题和情感（v5 重构版）
...
- 典型意象：懊道人、臣非老画师、两革科名一贬官、老夫卖画、落拓、潦倒、故山、归隐
- ⚠️ 这是李鱓最核心的主题——凡是涉及"我"的境遇感慨，都应归此类

**主题2（咏物寄兴）**：借所画之物抒发情志、寄托品格理想
- 核心判据：以画中物象为媒介，寄托品格、志向、心境（松竹喻品格、花鸟喻心境）
...
- ⚠️ "懊道人"署名是李鱓自嘲标志，即使题跋有吉祥词，仍可能是身世自况或反讽
- 必须返回至少2个主题（题跋少于5字除外）
...
- 典型意象：懊道人、臣非老画师、两革科名一贬官、老夫卖画、落拓、潦倒、故山、归隐、白发、迟暮
- ⚠️ 这是李鱓最核心的主题——凡是涉及"我"的境遇感慨，都应归此类
{artist_se_names}
...
- **易混淆情形**：
  - ⚠️ 含"懊道人"署名的吉语题跋，可能是身世自况（反讽），不是纯粹的吉语祥瑞
